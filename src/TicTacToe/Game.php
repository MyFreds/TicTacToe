...
class Game {
    ...
    public function start(): void {
        $this->sendBoard();
        $this->playerX->sendMessage("§7[§aTicTacToe§7] §2Game started! You are X. It's your turn.");
        $this->playerO->sendMessage("§7[§aTicTacToe§7] §2Game started! You are O. Waiting for the other player's move.");
    }

    public function handleMove(Player $player, int $position): void {
        if ($player !== $this->currentPlayer) {
            $player->sendMessage("§7[§aTicTacToe§7] §cIt's not your turn!");
            return;
        }

        if ($position < 1 || $position > 9 || $this->board[$position - 1] !== " ") {
            $player->sendMessage("§7[§aTicTacToe§7] §cInvalid move!");
            return;
        }
        ...
        $this->sendBoard();
        $this->currentPlayer->sendMessage("§7[§aTicTacToe§7] §fIt's your turn!");
        ($this->currentPlayer === $this->playerX ? $this->playerO : $this->playerX)->sendMessage("§7[§aTicTacToe§7] §eWaiting for the other player's move.");
    }
    ...
    private function formatBoard(): string {
        ...
        return TextFormat::AQUA . "TicTacToe\n" .
    ...
    public function end(?Player $winner = null): void {
        if ($winner !== null) {
            ...
            if ($economy !== null) {
                ...
                $winner->sendMessage("§7[§aTicTacToe§7] §2Congratulations! You won the game and received §a" . $reward . " §2money!");
           } else {
                $winner->sendMessage("§7[§aTicTacToe§7] §2Congratulations! You won the game!");
            }
            ($winner === $this->playerX ? $this->playerO : $this->playerX)->sendMessage("§7[§aTicTacToe§7] §cYou lost the game.");
        } else {
            $this->playerX->sendMessage("§7[§aTicTacToe§7] §eThe game ended in a draw.");
            $this->playerO->sendMessage("§7[§aTicTacToe§7] §eThe game ended in a draw.");
        }

        $this->playerX->sendMessage("§7[§aTicTacToe§7] §7Use /tictactoe start to start a new game.");
        $this->playerO->sendMessage("§7[§aTicTacToe§7] §7Use /tictactoe start to start a new game.");
